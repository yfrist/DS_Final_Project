...
def clean_data(data):
    ...
    data = data.drop(columns=['white_id', 'black_id', 'moves', 'opening_fullname',
...
def filter_data(data):
    ...
    """
    # Features relevant for clustering game types
    clustering_features = ['turns', 'opening_moves', 'base_time', 'increment', 'rating_diff']

    # Drop any rows with missing or invalid data in clustering features
    data = data.dropna(subset=clustering_features)

    # Normalize numerical features (optional but recommended for k-means)
    ...
    clustering_df = pd.DataFrame(data_scaled, columns=clustering_features)

    print("Data filtered and scaled for clustering.")
...
if __name__ == "__main__":
    ...
    if data is not None:
        ...
        # Save cleaned data
        save_data(filtered_data, "data/cleaned_chess_games_clusters.csv")
    else:
